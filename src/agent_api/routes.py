...
run_service.adapters.update(
    {
        ...
        "mock_import_declaration": MockImportDeclarationAdapter(
            MockCustomsWorkflowService(
...
def configure_customs_authority(llm_config: dict) -> None:
    adapter = run_service.adapters.get("mock_import_declaration")
    if isinstance(adapter, MockImportDeclarationAdapter):
        adapter.workflow.configure_authority(llm_config)
    ...
    return CapabilitiesResponse(
        capabilities=[
            CapabilityDescriptor(
                ...
                default_timeout_seconds=get_default_run_timeout("chat"),
            ...
            CapabilityDescriptor(
                ...
                output_kinds=["text", "structured_data"],
                default_timeout_seconds=get_default_run_timeout("audit"),
            ...
            CapabilityDescriptor(
                ...
                input_kinds=["image"],
                output_kinds=["text", "structured_data"],
                default_timeout_seconds=get_default_run_timeout("ocr"),
                silence_timeout_seconds=120,
                heartbeat_seconds=20,
            ),
            CapabilityDescriptor(
                ...
                default_timeout_seconds=get_default_run_timeout("report"),
            ...
            CapabilityDescriptor(
                ...
                default_timeout_seconds=get_default_run_timeout("full_review"),
                silence_timeout_seconds=180,
                heartbeat_seconds=20,
            ),
            CapabilityDescriptor(
                name="mock_import_declaration",
                description="持久化的一般贸易进口全链路模拟，覆盖退单、查验、征税、放行和结关",
                input_kinds=["text", "structured_data"],
                output_kinds=["text", "structured_data"],
                default_timeout_seconds=get_default_run_timeout(
            ...
            CapabilityDescriptor(
                ...
                status="degraded",
                default_timeout_seconds=get_default_run_timeout("batch_audit"),
                silence_timeout_seconds=180,
                heartbeat_seconds=20,
            ),
            CapabilityDescriptor(
