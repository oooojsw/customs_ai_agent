...
class ToolFailureAdapter:
    async def execute(self, request, app, emitter, context):
        await emitter.tool_started("failing_tool", "失败工具")
...
def test_run_service_uses_intent_default_timeout_when_not_explicit():
    async def scenario():
        service = AgentRunService(
            adapters={"report": FakeAdapter()}, heartbeat_seconds=60
        )
        request = CreateRunRequest.model_validate(
            {
                "request_id": "req-report-timeout-default",
                "session": {
                    "session_id": "session-report-timeout",
                    "user_id": "user-001",
                    "tenant_id": "tenant-001",
                },
                "message": {"content": "生成报告"},
                "options": {"intent": "report"},
            }
        )
        ...
        assert snapshot.status == RunStatus.COMPLETED
        assert snapshot.usage["configured_timeout_seconds"] == 1200
        assert snapshot.usage["timeout_source"] == "intent_default"

    asyncio.run(scenario())


def test_run_service_respects_explicit_timeout():
    async def scenario():
        service = AgentRunService(
            adapters={"report": FakeAdapter()}, heartbeat_seconds=60
        )
        request = CreateRunRequest.model_validate(
            {
                "request_id": "req-report-timeout-explicit",
                "session": {
                    "session_id": "session-report-timeout-explicit",
                    "user_id": "user-001",
                    "tenant_id": "tenant-001",
                },
                "message": {"content": "生成报告"},
                "options": {"intent": "report", "timeout_seconds": 45},
            }
        )
        response = await service.create_run(request, FastAPI())
        snapshot = await wait_for_terminal(service, response.run_id)

        assert snapshot.usage["configured_timeout_seconds"] == 45
        assert snapshot.usage["timeout_source"] == "request"
...
def test_failed_run_closes_active_tool_before_terminal_event():
    async def scenario():
        store = InMemoryRunStore()
        service = AgentRunService(
            store=store,
            adapters={"chat": ToolFailureAdapter()},
            heartbeat_seconds=60,
        )
        created = await service.create_run(
            make_request(request_id="req-tool-failure-001"),
            FastAPI(),
        )
        snapshot = await wait_for_terminal(service, created.run_id)
        events = await store.events_after(created.run_id)
